...
/*
  Ozon Seller Analytics — Seller API + validated supplements sync (v10.8 Period-level Ozon Expenses)

  Source policy:
    - operational Ozon data comes from Seller API;
    - validated business supplements are allowed for marketplace-buyout/CIS commission rows
      and for compensation-to-posting links that Ozon omits from Seller API;
    - the supplement adds commission expenses and may supply revenue only for a
      validated CIS posting that is completely absent from Seller API delivered sales;
...
const FINANCE_ATTRIBUTION_VERSION = 12;
...
// Never compare those strings lexicographically before canonicalising them.
const dateOnly = v => {
...
const FINANCE_COMPONENT_KEYS=['commission','acquiring','logistics','storage','ads','fines','returns','other'];
function commonOzonExpenseGroup(line,row={}){
  // Charge-line names are authoritative. Row-level operation text is only a
  // fallback for legacy rows without a detailed charge breakdown; otherwise a
  // mixed accrual titled "fine" could incorrectly pull its sales commission out
  // of the order together with the actual fine line.
  const ownText=asStr(line?.typeName).trim();
  const genericLine=/^корректировка сверки начисления$|^type \d+$|^other$|^прочее$/i.test(ownText);
  const text=(ownText&&!genericLine?ownText:`${ownText} ${asStr(row.group)} ${asStr(row.operation)}`).toLowerCase().replaceAll('ё','е');
  const component=asStr(line?.component);
  if(/premium|премиум/.test(text))return'Премиум-подписка';
...
function splitCommonOzonExpenses(financeRows){
  ...
  for(const source of financeRows||[]){
    const suppliedLines=Array.isArray(source.chargeLines)&&source.chargeLines.length?source.chargeLines:null;
    const lines=suppliedLines||FINANCE_COMPONENT_KEYS.filter(k=>Math.abs(asNum(source[k],0))>.0005).map(k=>({typeId:null,typeName:first(source.operation,source.group,k),component:k,amount:asNum(source[k],0)}));
    const commonLines=[],orderLines=[];
    for(const line of lines){
      const managementGroup=commonOzonExpenseGroup(line,source);
    ...
    for(const line of commonLines){
      ...
      commonComponents[component]+=asNum(line.amount,0);
    }
    const commonNet=-FINANCE_COMPONENT_KEYS.reduce((z,k)=>z+asNum(commonComponents[k],0),0);
    const normalComponents=Object.fromEntries(FINANCE_COMPONENT_KEYS.map(k=>[k,asNum(source[k],0)-asNum(commonComponents[k],0)]));
    const normalNet=asNum(source.rawAmount,0)-commonNet;
    const normalHasValue=Math.abs(normalNet)>.0005||Math.abs(asNum(source.grossRevenue,0))>.0005||FINANCE_COMPONENT_KEYS.some(k=>Math.abs(normalComponents[k])>.0005);
    if(normalHasValue){
      out.push({...source,...normalComponents,rawAmount:normalNet,chargeLines:orderLines,transactionId:`${asStr(source.transactionId)}:order`});
      diagnostics.splitRows++;
    }
    const groups=[...new Set(commonLines.map(x=>x.managementGroup).filter(Boolean))];
    const managementGroup=groups.length===1?groups[0]:'Общие расходы Ozon';
    const commonRow={...source,...commonComponents,
      date:dateOnly(first(source.financeDate,source.date)),financeDate:dateOnly(first(source.financeDate,source.date)),
      article:'',sku:'',itemSkus:[],linkedProducts:[],grossRevenue:null,sellerPriceRaw:0,salePriceRaw:0,saleAmount:0,bonus:0,coinvestment:0,
      soldQty:0,returnedQty:0,financeQtyResolved:0,financeQtyUnresolved:0,feeQuantity:0,compensationIncome:0,financeIncomeKind:'',
      rawAmount:commonNet,chargeLines:commonLines,financeAttribution:'unallocated',financeScope:'period',managementScope:'period',commonExpense:true,commonExpenseGroup:managementGroup,
      sourceArticle:asStr(source.article),sourceSku:asStr(source.sku),sourcePostingNumber:asStr(source.postingNumber),sourceOrderNumber:asStr(source.orderNumber),
      transactionId:`${asStr(source.transactionId)}:period-common`
    };
    out.push(commonRow);
    diagnostics.commonRows++;
    diagnostics.commonNet+=commonNet;
    diagnostics.commonExpenses+=FINANCE_COMPONENT_KEYS.reduce((z,k)=>z+Math.max(0,asNum(commonComponents[k],0)),0);
    for(const line of commonLines){
      const group=line.managementGroup||managementGroup;
      diagnostics.groups[group]=(diagnostics.groups[group]||0)+asNum(line.amount,0);
      const month=monthKey(commonRow.financeDate);
      diagnostics.months[month]=diagnostics.months[month]||{};
      diagnostics.months[month][group]=(diagnostics.months[month][group]||0)+asNum(line.amount,0);
    }
  }
...
function attributeOrderFinanceToSaleDate(financeRows,realizedRows){
  ...
  const diagnostics={rows:0,linkedRows:0,shiftedRows:0,returnRowsKeptOnFinanceDate:0,positiveIncomeRowsKeptOnFinanceDate:0,periodCommonRowsKeptOnFinanceDate:0,linkedWithoutSaleDate:0,shiftedNet:0,shiftedExpenses:0,monthMoves:{}};
  ...
  const rows=(financeRows||[]).map(r=>{
    ...
    const periodCommon=Boolean(r.commonExpense||r.managementScope==='period');
    ...
    if(periodCommon)diagnostics.periodCommonRowsKeptOnFinanceDate++;
    const useSaleDate=linked&&!returnRelated&&!positiveIncome&&!periodCommon&&grossExpense>0.0005&&saleDate;
...
async function loadCompensationLinkSupplement(){
  const file=path.join(process.cwd(),'data','compensation-links.json');
  try{
    const doc=JSON.parse(await fs.readFile(file,'utf8'));
    if(doc?.kind!=='ozon_compensation_link_supplement'||!Array.isArray(doc?.rows))throw new Error('unexpected supplement schema');
    const rows=doc.rows.map((r,i)=>({
      id:asStr(r.id||`compensation-${i+1}`),period:asStr(r.period),postingNumber:asStr(r.postingNumber),
      amount:Math.abs(asNum(r.amount,0)),name:asStr(r.name),source:asStr(r.source||doc.sourceFile)
    })).filter(r=>/^\d{4}-\d{2}$/.test(r.period)&&r.postingNumber&&r.amount>0);
    if(rows.length!==doc.rows.length)throw new Error(`invalid rows: ${doc.rows.length-rows.length}`);
    return {rows,meta:doc,total:rows.reduce((z,r)=>z+r.amount,0)};
  }catch(e){
    if(e?.code==='ENOENT')return {rows:[],meta:null,total:0};
    throw new Error(`Compensation link supplement: ${e?.message||e}`);
  }
}

function applyValidatedCompensationLinks(financeRows,supplement,postingMap,maps){
  const componentKeys=['commission','acquiring','logistics','storage','ads','fines','returns','other'];
  ...
  for(const spec of supplement?.rows||[]){
    ...
    if(Math.abs(naturalTotal-spec.amount)<=0.01){
      diagnostics.alreadyLinked++;
      diagnostics.total+=naturalTotal;
      continue;
    }
    ...
    })).filter(p=>p.sku||p.article);
    if(!products.length)throw new Error(`Compensation ${spec.id}: posting ${spec.postingNumber} has no product composition in Posting API history`);

    // Remove only the matched positive amount from the anonymous period row. The
    // same amount is recreated below with the posting/SKU link, so the Finance
    // ledger total stays exactly unchanged.
    source.rawAmount=asNum(source.rawAmount,0)-spec.amount;
    let remaining=spec.amount;
    for(const k of componentKeys){
      const available=Math.max(0,-asNum(source[k],0));
      const take=Math.min(available,remaining);
      if(take>0){source[k]=asNum(source[k],0)+take;remaining-=take}
      if(remaining<=0.005)break;
    }
    if(remaining>0.01)throw new Error(`Compensation ${spec.id}: anonymous Finance components do not cover ${spec.amount.toFixed(2)}`);
    source.chargeLines=[];
    diagnostics.sourceRowsConsumed++;

    const weights=products.map(p=>Math.max(0,p.price*p.quantity)||Math.max(0,p.quantity)||1);
    const weightTotal=weights.reduce((z,x)=>z+x,0)||products.length;
    products.forEach((p,i)=>{
      const share=spec.amount*weights[i]/weightTotal;
      rows.push({
        date:source.date,article:p.article,sku:p.sku,name:p.name,postingNumber:spec.postingNumber,financeUnitNumber:spec.postingNumber,
        orderId:asStr(meta?.orderId),orderNumber:asStr(meta?.orderNumber),orderDate:asStr(meta?.orderDate),orderDateSource:asStr(meta?.orderDateSource),orderSchema:asStr(meta?.orderSchema),
        itemSkus:[p.sku].filter(Boolean),group:'Компенсации Ozon',operation:'Компенсация по валидированному отчёту Ozon',
        transactionId:`validated-compensation-${spec.id}-${i+1}`,grossRevenue:null,sellerPriceRaw:0,salePriceRaw:0,saleAmount:0,bonus:0,coinvestment:0,
        soldQty:0,returnedQty:0,financeQtyResolved:0,financeQtyUnresolved:0,...emptyFinanceComponents(),other:-share,rawAmount:share,
        compensationIncome:share,financeIncomeKind:'compensation',financeSource:'finance/accrual/by-day + validated compensation link',financeAttribution:'direct_sku',financeScope:'posting_item',
        feeQuantity:0,chargeLines:[{typeId:null,typeName:'Компенсация Ozon',component:'other',amount:-share,incomeKind:'compensation'}],
        parentAccrualId:source.parentAccrualId||source.transactionId,validatedCompensationId:spec.id,manualSupplement:true,sourceReference:spec.source
      });
    });
    diagnostics.linked++;
    diagnostics.total+=spec.amount;
    diagnostics.links.push({id:spec.id,postingNumber:spec.postingNumber,date:source.date,amount:spec.amount,products:products.length});
  }
  return {rows:rows.filter(r=>Math.abs(asNum(r.rawAmount,0))>0.005||componentKeys.some(k=>Math.abs(asNum(r[k],0))>0.005)||asNum(r.grossRevenue,0)),diagnostics};
}
...
async function fetchFboPostings(fromDate,toDate){
  ...
  const rows=await fetchPostingPages('/v3/posting/fbo/list',(from,to,cursor)=>({
    cursor,
  ...
  return rows.map(x=>({...x,__postingSource:'FBO'}));
}
async function fetchFbsPostings(fromDate,toDate){
  ...
  const rows=await fetchPostingPages('/v4/posting/fbs/list',(from,to,cursor)=>({
    sort_dir:'asc',
    filter:{since:rfc3339Start(from),to:rfc3339End(to)},
    limit:100,
    cursor,
  ...
  return rows.map(x=>({...x,__postingSource:'FBS'}));
}
...
async function fetchFbsDeliveredStatusDay(date){
  ...
  for(let guard=0;guard<500;guard++){
    ...
    for(const row of batch)out.push({...row,__exactDeliveredDate:date,__postingSource:'FBS'});
  ...
  for(const p of postings||[]){
    ...
    out[num]={
      ...
      products:(p.products||[]).map(x=>{
        const rawPrice=first(x.price,x.marketplace_seller_price);
        return {
          ...
          quantity:Math.max(0,asNum(x.quantity,0)),price:money(rawPrice),pricePresent:rawPrice!==undefined&&rawPrice!==null&&rawPrice!==''
        };
      }).filter(x=>x.article||x.sku)
    };
  } return out;
...
function applyFinanceRecognitionDates(rows,financeRows){
  const idx=new Map();
  for(const f of financeRows||[]){
    const amount=asNum(first(f.saleAmount,f.grossRevenue),0);if(amount<=0.00001)continue;
    const pn=asStr(f.postingNumber),sku=asStr(f.sku),article=asStr(f.article),date=dateOnly(f.date);if(!pn||(!sku&&!article)||!date)continue;
    for(const productId of [sku,article].filter(Boolean)){
      const key=`${pn}|${productId}`,dates=idx.get(key)||new Set();dates.add(date);idx.set(key,dates);
    }
  }
  let financeDated=0,fallbackDated=0,ambiguous=0;
  const recognized=(rows||[]).map(r=>{
    const pn=asStr(r.postingNumber),sku=asStr(r.sku),article=asStr(r.article);
    const dates=[...(idx.get(`${pn}|${sku}`)||idx.get(`${pn}|${article}`)||[])].sort();
    if(dates.length){
      financeDated++;if(dates.length>1)ambiguous++;
      return {...r,date:dates[0],recognitionDate:dates[0],recognitionDateSource:dates.length===1?'finance-positive-sale-accrual':'finance-positive-sale-accrual-first',financeSaleDates:dates,dateAttribution:'finance-positive-sale-accrual'};
    }
    fallbackDated++;
    const fallback=dateOnly(r.date||r.deliveryDate);
    return {...r,date:fallback,recognitionDate:fallback,recognitionDateSource:asStr(r.source).includes('marketplace-buyout')?'validated-marketplace-buyout-date':'delivery-date-fallback-no-positive-finance-sale',financeSaleDates:[]};
  });
  return {rows:recognized,financeDated,fallbackDated,ambiguous};
}
function supplementReturnsFromReturnsApi(returnEvents,returnRows,deliveredRows,postingMap,maps){
  const out=[...(returnEvents||[])],soldKeys=new Set((deliveredRows||[]).map(r=>postingSkuKey(r.postingNumber,r.sku,r.article)));
  const existing=new Map();
  for(const r of out){const key=postingSkuKey(r.postingNumber,r.sku,r.article);existing.set(key,(existing.get(key)||0)+Math.max(0,asNum(r.returnedQty,0)))}
  const grouped=new Map();
  for(const r of returnRows||[]){
    const key=postingSkuKey(r.postingNumber,r.sku,r.article);if(!soldKeys.has(key))continue;
    const g=grouped.get(key)||{qty:0,dates:[],sample:r};g.qty+=Math.max(0,asNum(r.quantity,0));if(r.date)g.dates.push(dateOnly(r.date));grouped.set(key,g);
  }
  let addedRows=0,addedUnits=0;
  for(const [key,g] of grouped){
    const missing=Math.max(0,g.qty-(existing.get(key)||0));if(missing<=0.00001)continue;
    const r=g.sample,pm=postingMap?.[asStr(r.postingNumber)]||{},date=g.dates.filter(Boolean).sort().at(-1)||dateOnly(r.date);
    out.push({date,deliveryDate:'',deliveryDateExact:null,deliveryDateSource:'',returnDate:date,article:asStr(first(r.article,maps.articleBySku.get(asStr(r.sku)))),sku:asStr(r.sku),name:asStr(first(r.name,maps.nameBySku.get(asStr(r.sku)))),soldQty:0,returnedQty:missing,netQty:-missing,postingNumber:asStr(r.postingNumber),orderId:asStr(pm.orderId),orderNumber:asStr(pm.orderNumber),orderDate:dateOnly(pm.orderDate),orderDateSource:asStr(pm.orderDateSource),orderSchema:asStr(first(pm.orderSchema,r.schema)),unitPrice:null,source:'returns-api-supplement',officialQuantity:true,dateAttribution:'returns-api-return-date'});
    addedRows++;addedUnits+=missing;
  }
  return {rows:out,addedRows,addedUnits};
}
...
const USER_AUDIT_POSTINGS=[
  '88070354-0036-1','99068459-0640-1','0102865916-0364-1','0141461295-0047-1','0153848164-0007-1','0150691258-0084-1',
  '0150691258-0086-1','0150691258-0085-1','0148172677-0022-1','0126377987-0017-1','0157019882-0043-1','0243441369-0052-5',
  '0115309418-0086-3','97416390-0165-1','0105975830-0018-1','0114743909-0663-1','09510564-0233-1','71218150-0111-1',
  '0203561815-0013-1','0247364885-0057-10','0159430985-0034-4','0202501690-0187-2','0187578693-0194-1','0154180798-0032-1'
];
function postingSkuKey(postingNumber,sku,article=''){
  return `${asStr(postingNumber)}|${asStr(sku)||`A:${asStr(article)}`}`;
}
function buildDeliveredItemEvidence(financeRows=[],controlRows=[]){
  // Posting status `delivered` is shipment-level. In a multi-product FBS posting Ozon can
  // still have an individual product that was not economically realized (for example a
  // line that only generated reverse-logistics activity). v9.1 therefore verifies only
  // ambiguous multi-product lines at posting+SKU level. We DO NOT require a Finance sale
  // for every delivered item because some valid BuyoutDetails deliveries have no ordinary
  // `Продажи → Выручка` accrual line. Negative evidence is used only when it is explicit.
  const out=new Map(),officialSoldPostings=new Set();
  const ensure=(pn,sku,article)=>{
    const k=postingSkuKey(pn,sku,article);
    if(!out.has(k))out.set(k,{positiveSale:false,officialSold:false,reverseEvidence:false,financeRows:0});
    return out.get(k);
  };
  for(const r of financeRows||[]){
    const pn=asStr(r.postingNumber),sku=asStr(r.sku),article=asStr(r.article);if(!pn||(!sku&&!article))continue;
    const e=ensure(pn,sku,article);e.financeRows++;
    if(asNum(r.saleAmount,0)>0.005)e.positiveSale=true;
    const txt=`${asStr(r.operation)} ${asStr(r.group)}`.toLowerCase();
    if(asNum(r.saleAmount,0)<-0.005||asNum(r.returnedQty,0)>0||asNum(r.returns,0)>0.005||/обратн|reverse|возврат|return|отмен|cancel/.test(txt))e.reverseEvidence=true;
  }
  for(const r of controlRows||[]){
    const pn=asStr(r.postingNumber),sku=asStr(r.sku),article=asStr(r.article);if(!pn||(!sku&&!article))continue;
    const e=ensure(pn,sku,article);if(asNum(r.soldQty,0)>0){e.officialSold=true;officialSoldPostings.add(pn)}
  }
  return {byItem:out,officialSoldPostings};
}
function buildDeliveredSalesFromPostingMap(postingMap,start,end,maps,financeRows=[],controlRows=[]){
  const rows=[];const excludedRows=[];let missingPriceRows=0,nonFbsDeliveredWithoutExactDate=0;
  const evidence=buildDeliveredItemEvidence(financeRows,controlRows);
  for(const pm of Object.values(postingMap||{})){
    if(asStr(pm.status).toLowerCase()!=='delivered')continue;
    const date=dateOnly(pm.observedDeliveredDate),source=asStr(pm.postingSource).toUpperCase();
    if(!date){
      if(source&&source!=='FBS')nonFbsDeliveredWithoutExactDate++;
      continue;
    }
    if(date<start||date>end)continue;
    const products=(pm.products||[]).filter(prod=>Math.max(0,asNum(prod.quantity,0))>0);
    const multiProduct=products.length>1;
    for(const prod of products){
      const q=Math.max(0,asNum(prod.quantity,0));if(q<=0)continue;
      const sku=asStr(prod.sku),article=asStr(first(prod.article,maps.articleBySku.get(sku)));if(!sku&&!article)continue;
      const pn=asStr(pm.postingNumber),ev=evidence.byItem.get(postingSkuKey(pn,sku,article));
      // Crucial v9.1 rule: a delivered SHIPMENT does not prove that every line inside a
      // multi-product shipment was sold. Exclude only when Ozon API gives affirmative
      // item-level reverse/cancellation evidence AND there is no positive sale evidence
      // from either Finance or realization controls. This preserves legitimate special
      // deliveries that simply have no ordinary Finance revenue accrual.
      // If the official realization contains at least one sold line for this posting,
      // it is authoritative for the item composition of that multi-product posting.
      // This closes the gap where a cancelled second line had no explicit reverse row
      // and was previously counted merely because the shipment itself was delivered.
      const excludedByOfficialComposition=multiProduct&&evidence.officialSoldPostings.has(pn)&&!ev?.officialSold;
      const excludedByReverseEvidence=multiProduct&&ev?.reverseEvidence&&!ev?.positiveSale&&!ev?.officialSold;
      const exclude=Boolean(excludedByOfficialComposition||excludedByReverseEvidence);
      if(exclude){
        excludedRows.push({postingNumber:pn,orderNumber:asStr(pm.orderNumber),deliveryDate:date,article,sku,name:asStr(first(prod.name,maps.nameBySku.get(sku))),quantity:q,price:asNum(prod.price,0),reason:excludedByOfficialComposition?'multi-product delivered posting: item absent from official sold composition':'multi-product delivered posting: reverse/cancel evidence without positive item-level sale evidence'});
        continue;
      }
      const price=asNum(prod.price,NaN),pricePresent=prod.pricePresent!==false&&Number.isFinite(price);
      if(!pricePresent)missingPriceRows++;
      rows.push({
        date,deliveryDate:date,deliveryDateExact:true,deliveryDateSource:asStr(pm.observedDeliveredDateSource)||'fbs-last-changed-status-date',returnDate:'',
        article,sku,name:asStr(first(prod.name,maps.nameBySku.get(sku))),soldQty:q,returnedQty:0,netQty:q,
        postingNumber:asStr(pm.postingNumber),orderId:asStr(pm.orderId),orderNumber:asStr(pm.orderNumber),orderDate:dateOnly(pm.orderDate),orderDateSource:asStr(pm.orderDateSource),orderSchema:asStr(pm.orderSchema),
        unitPrice:pricePresent?price:null,unitPriceSource:pricePresent?'posting-product-price':'missing',source:'delivered-posting-v9.1-item-verified',officialQuantity:false,dateAttribution:'fbs-last-changed-status-date',provisional:monthKey(date)===monthKey(TODAY)
      });
    }
  }
  rows.sort((a,b)=>a.deliveryDate.localeCompare(b.deliveryDate)||asStr(a.postingNumber).localeCompare(asStr(b.postingNumber))||asStr(a.sku).localeCompare(asStr(b.sku)));
  excludedRows.sort((a,b)=>a.deliveryDate.localeCompare(b.deliveryDate)||a.postingNumber.localeCompare(b.postingNumber)||a.sku.localeCompare(b.sku));
  return {rows,excludedRows,excludedDeliveredProductRows:excludedRows.length,excludedDeliveredProductUnits:excludedRows.reduce((z,r)=>z+asNum(r.quantity,0),0),excludedDeliveredProductGross:excludedRows.reduce((z,r)=>z+asNum(r.quantity,0)*asNum(r.price,0),0),missingPriceRows,nonFbsDeliveredWithoutExactDate};
}
function appendMissingMarketplaceBuyoutSales(deliveredRows,buyoutRows,postingMap,start,end,maps){
  const rows=[...(deliveredRows||[])];
  const existingPostings=new Set(rows.map(r=>asStr(r.postingNumber)).filter(Boolean)),addedKeys=new Set();
  const added=[],invalid=[];
  for(const f of buyoutRows||[]){
    const b=f.marketplaceBuyout||{},pn=asStr(f.postingNumber),date=dateOnly(f.date),qty=Math.max(0,asNum(b.quantity,0)),gross=Math.max(0,asNum(b.sellerPrice,0));
    if(!pn||!date||date<start||date>end||existingPostings.has(pn))continue;
    if(!(qty>0&&gross>0)){invalid.push({postingNumber:pn,date,quantity:qty,sellerPrice:gross});continue}
    const pm=postingMap?.[pn]||{},sku=asStr(f.sku),article=asStr(first(f.article,maps.articleBySku.get(sku))),itemKey=`${pn}|${sku||`A:${article}`}`;if(addedKeys.has(itemKey))continue;const unit=gross/qty;
    const row={
      date,deliveryDate:date,deliveryDateExact:true,deliveryDateSource:'validated-marketplace-buyout-date',returnDate:'',
      article,sku,name:asStr(first(f.name,maps.nameBySku.get(sku))),soldQty:qty,returnedQty:0,netQty:qty,
      postingNumber:pn,orderId:asStr(pm.orderId),orderNumber:asStr(pm.orderNumber),orderDate:dateOnly(pm.orderDate),orderDateSource:asStr(pm.orderDateSource),orderSchema:asStr(first(pm.orderSchema,'CIS')),
      unitPrice:unit,unitPriceSource:'validated-marketplace-buyout-seller-price',source:'validated-marketplace-buyout-sale-fallback',officialQuantity:true,dateAttribution:'validated-marketplace-buyout-date',provisional:false
    };
    rows.push(row);added.push(row);addedKeys.add(itemKey);
  }
  rows.sort((a,b)=>asStr(a.deliveryDate).localeCompare(asStr(b.deliveryDate))||asStr(a.postingNumber).localeCompare(asStr(b.postingNumber))||asStr(a.sku).localeCompare(asStr(b.sku)));
  const coveredPostings=new Set(rows.map(r=>asStr(r.postingNumber)).filter(Boolean));
  const expectedPostings=new Set((buyoutRows||[]).map(r=>asStr(r.postingNumber)).filter(Boolean));
  const missingPostings=[...expectedPostings].filter(pn=>!coveredPostings.has(pn)).sort();
  return {rows,added,invalid,expectedPostings:expectedPostings.size,coveredPostings:expectedPostings.size-missingPostings.length,missingPostings};
}
function validateRealizationPublishGate(realization){
  const rows=realization?.rows||[],diag=realization?.diagnostics||{},seen=new Set();
  const duplicateKeys=[],invalidNumericRows=[],returnOverSaleRows=[],revenueBridgeRows=[];
  for(const r of rows){
    const key=`${asStr(r.postingNumber)}|${asStr(r.sku)||`A:${asStr(r.article)}`}|${dateOnly(r.deliveryDate)}`;
    if(seen.has(key))duplicateKeys.push(key);else seen.add(key);
    const sold=asNum(r.soldQty,NaN),returned=asNum(r.returnedQty,NaN),net=asNum(r.netQty,NaN),unit=asNum(r.unitPrice,NaN),revenue=asNum(r.revenue,NaN);
    if(!Number.isFinite(sold)||!Number.isFinite(returned)||!Number.isFinite(net)||!Number.isFinite(revenue)||(sold>0&&!Number.isFinite(unit)))invalidNumericRows.push(key);
    if(Number.isFinite(sold)&&Number.isFinite(returned)&&Number.isFinite(net)&&(returned>sold+0.0001||net<-0.0001||Math.abs((sold-returned)-net)>0.0001))returnOverSaleRows.push(key);
    if(Number.isFinite(net)&&Number.isFinite(unit)&&Number.isFinite(revenue)&&Math.abs(net*unit-revenue)>0.011)revenueBridgeRows.push(key);
  }
  const missingBuyout=Array.isArray(diag.marketplaceBuyoutSalesMissingPostings)?diag.marketplaceBuyoutSalesMissingPostings:[];
  const criticalIssues=[];
  if(duplicateKeys.length)criticalIssues.push(`duplicate delivered order-item rows: ${duplicateKeys.length}`);
  if(invalidNumericRows.length)criticalIssues.push(`invalid delivered numeric rows: ${invalidNumericRows.length}`);
  if(returnOverSaleRows.length)criticalIssues.push(`returns exceed sales or net bridge is broken: ${returnOverSaleRows.length}`);
  if(revenueBridgeRows.length)criticalIssues.push(`revenue != net quantity × unit price: ${revenueBridgeRows.length}`);
  if(asNum(diag.pAndLPriceMissing,0)>0)criticalIssues.push(`delivered rows without price: ${asNum(diag.pAndLPriceMissing,0)}`);
  if(asNum(diag.marketplaceBuyoutSalesInvalidRows,0)>0)criticalIssues.push(`invalid validated marketplace-buyout rows: ${asNum(diag.marketplaceBuyoutSalesInvalidRows,0)}`);
  if(missingBuyout.length)criticalIssues.push(`validated marketplace-buyout postings missing from revenue: ${missingBuyout.length}`);
  return {
    status:criticalIssues.length?'blocked':'passed',checkedAt:new Date().toISOString(),rowCount:rows.length,
    duplicateRows:duplicateKeys.length,invalidNumericRows:invalidNumericRows.length,returnOverSaleRows:returnOverSaleRows.length,revenueBridgeRows:revenueBridgeRows.length,
    marketplaceBuyoutExpectedPostings:diag.marketplaceBuyoutSalesExpectedPostings||0,marketplaceBuyoutCoveredPostings:diag.marketplaceBuyoutSalesCoveredPostings||0,
    criticalIssues,samples:{duplicates:duplicateKeys.slice(0,10),invalidNumeric:invalidNumericRows.slice(0,10),returnOverSale:returnOverSaleRows.slice(0,10),revenueBridge:revenueBridgeRows.slice(0,10),missingBuyout:missingBuyout.slice(0,10)}
  };
}
function deliveredPostingStats(rows,start,end){
  const rr=(rows||[]).filter(r=>r.deliveryDate>=start&&r.deliveryDate<=end);
  return {
    postings:new Set(rr.map(r=>r.postingNumber).filter(Boolean)).size,
    productRows:rr.length,
    sku:new Set(rr.map(r=>r.sku||r.article).filter(Boolean)).size,
    units:rr.reduce((z,r)=>z+asNum(r.soldQty,0),0),
    gross:rr.reduce((z,r)=>z+asNum(r.soldQty,0)*asNum(r.unitPrice,0),0),
    missingPriceRows:rr.filter(r=>!Number.isFinite(asNum(r.unitPrice,NaN))).length
  };
}
function logUserPostingAudit(postingMap){
  let found=0,delivered=0,gross=0;
  for(const pn of USER_AUDIT_POSTINGS){
    const pm=postingMap?.[pn];
    if(!pm){console.log(`USER POSTING AUDIT v9 ${pn}: found=false`);continue}
    found++;const isDelivered=asStr(pm.status).toLowerCase()==='delivered';if(isDelivered)delivered++;
    const products=pm.products||[],units=products.reduce((z,p)=>z+asNum(p.quantity,0),0),revenue=products.reduce((z,p)=>z+asNum(p.quantity,0)*asNum(p.price,0),0);gross+=isDelivered?revenue:0;
    console.log(`USER POSTING AUDIT v9 ${pn}: found=true; source=${asStr(pm.postingSource)||'unknown'}; status=${asStr(pm.status)}; delivery=${dateOnly(pm.observedDeliveredDate)||'none'}; order=${asStr(pm.orderNumber)||'none'}; orderDate=${dateOnly(pm.orderDate)||'none'}; productRows=${products.length}; units=${units}; postingRevenue=${revenue.toFixed(2)}`);
  }
  console.log(`USER POSTING AUDIT v9 summary: requested=${USER_AUDIT_POSTINGS.length}; found=${found}; delivered=${delivered}; deliveredPostingRevenue=${gross.toFixed(2)}`);
}
...
function postingsNeedingAccrual(targetGroups,financeIndex){
  // v9 sale timing comes from the actual delivered posting. Accrual/postings is
  // needed only to disambiguate RETURN identity/date when Finance by-day is not enough.
  const need=new Set();let parts=0;
  const emptyAccrual=new Map();
  for(const g of targetGroups||[])for(const t of g.targets||[]){
    const kind='return',qty=asNum(t.returnedQty,0);if(qty<=0)continue;
...
function logOrphanReturnDiagnostics(diags){
  for(let i=0;i<(diags||[]).length;i++){
    ...
    console.log(`UNMATCHED RETURN DIAGNOSTIC v9.0 #${n}: return=${JSON.stringify(d.returnEvent)}`);
    console.log(`UNMATCHED RETURN DIAGNOSTIC v9.0 #${n}: posting=${JSON.stringify(d.posting)}`);
    console.log(`UNMATCHED RETURN DIAGNOSTIC v9.0 #${n}: sourceRowsExact=${JSON.stringify(d.sourceRowsExact)}`);
    if(d.sourceRowsSamePostingOtherProduct?.length)console.log(`UNMATCHED RETURN DIAGNOSTIC v9.0 #${n}: sourceRowsSamePostingOtherProduct=${JSON.stringify(d.sourceRowsSamePostingOtherProduct)}`);
    console.log(`UNMATCHED RETURN DIAGNOSTIC v9.0 #${n}: financeExactNegative=${JSON.stringify(d.financeExactNegative)}`);
    console.log(`UNMATCHED RETURN DIAGNOSTIC v9.0 #${n}: financeExactPositive=${JSON.stringify(d.financeExactPositive)}`);
    console.log(`UNMATCHED RETURN DIAGNOSTIC v9.0 #${n}: financePriorSameProduct=${JSON.stringify(d.financePriorSameProduct)}`);
    console.log(`UNMATCHED RETURN DIAGNOSTIC v9.0 #${n}: returnsListExact=${JSON.stringify(d.returnsListExact)}`);
  }
}
function applyRetroactiveReturns(eventRows,postingMap={},businessStart=BUSINESS_START,deliveryContext={},returnRowsAudit=[]){
  ...
  const exactHistoryStart=dateOnly(deliveryContext?.historyStart);
  const exactFbsHistoryComplete=deliveryContext?.fbsComplete===true;
  for(const [key,rr] of returns)if(!sales.has(key)){
    for(const r of rr){
      const pn=asStr(r.postingNumber),sku=asStr(r.sku),article=asStr(r.article);
      const pm=postingMap?.[pn]||{};
      const delivered=dateOnly(pm.observedDeliveredDate);
      const orderDate=dateOnly(first(r.orderDate,pm.orderDate));
      const audit=(returnRowsAudit||[]).find(x=>asStr(x.postingNumber)===pn&&((sku&&asStr(x.sku)===sku)||(article&&asStr(x.article)===article)));
      const schema=asStr(first(r.orderSchema,pm.orderSchema,audit?.schema)).toUpperCase();
      if(delivered&&delivered<businessStart){
        prePeriodReturns.push({...r,sourceDeliveryDate:delivered,prePeriodProof:'exact-delivery-date'});
      }else if(!delivered&&exactFbsHistoryComplete&&exactHistoryStart&&exactHistoryStart<businessStart&&schema==='FBS'&&orderDate&&orderDate<exactHistoryStart){
        // Proof by exclusion: exact FBS delivered-status history is complete from
        // exactHistoryStart onward. This posting has a genuine FBS return, was ordered
        // before that window, and never appears as delivered inside the complete window.
        // Therefore its source delivery predates exactHistoryStart, hence BUSINESS_START.
        prePeriodReturns.push({...r,sourceDeliveryDate:'',prePeriodProof:'complete-fbs-history-exclusion',proofHistoryStart:exactHistoryStart,orderDate,schema});
...
async function buildDailyBusinessRealization(previous,maps,financeRows,postingMap,returnRows,returnsComplete,warnings,deliveryHistoryContext={},validatedBuyoutRows=[]){
  ...
  // Closed months are rebuilt once for the realization-control engine v9, then persisted. The stored rows are
  ...
  console.log(`Quantity control v9 date attribution: financeEventKeys=${financeIndex.size}; ambiguousPartsBeforeAccrual=${accrualNeed.parts}; accrualPostingsRequested=${accrualNeed.postingNumbers.length}`);
  ...
  for(const x of freshGroups){
    ...
    if(x.closed){
      // Monthly realization is a CONTROL on its own accounting basis. We do not need
      // a sale event date for P&L anymore; only return events need an identity/date so
      // they can retroactively reduce the original delivered posting.
      const controlRows=postingTargetsAsRows(x.targets,x.ym);
      const sourceValidation=validationBetweenRealizedRows(controlRows,x.monthly.rows,bounds.start,bounds.end);
      const returnRowsForPnl=built.rows.filter(r=>asNum(r.returnedQty,0)>0);
      const unresolvedReturns=built.unresolved.filter(u=>u.kind==='return');
      const complete=Boolean(x.postingValidation?.ok&&unresolvedReturns.length===0&&sourceValidation.ok);
      if(unresolvedReturns.length)warnings.push(`Return control ${x.ym}: ${unresolvedReturns.length} return quantity parts have no unambiguous posting/date identity.`);
      console.log(`Quantity control month v9 ${x.ym}: controlRows=${controlRows.length}; returnEventRows=${returnRowsForPnl.length}; unresolvedReturns=${unresolvedReturns.length}; sourceMonthly=${sourceValidation.ok}; complete=${complete}`);
      const seg={key:`Q9:${x.ym}`,kind:'quantity-month',month:x.ym,engineVersion:QUANTITY_ENGINE_VERSION,start:bounds.start,end:bounds.end,complete,officialQuantity:true,source:'v9 realization/posting monthly control; sales P&L comes from exact delivered postings',rows:controlRows,returnRows:returnRowsForPnl,postingValidation:x.postingValidation,sourceMonthlyValidation:sourceValidation,reasonCounts:built.reasonCounts};
      quantityMonthSegments.push(seg);monthDiagnostics.push({month:x.ym,reused:false,complete,postingValidation:x.postingValidation,sourceMonthlyValidation:sourceValidation,reasonCounts:built.reasonCounts,unresolvedReturns:unresolvedReturns.length});
    }else{
      currentEventRows=built.rows.filter(r=>r.date>=monthStart(TODAY)&&r.date<=targetEnd&&asNum(r.returnedQty,0)>0);
      currentUnresolved=built.unresolved.filter(u=>u.kind==='return').length;
      if(currentUnresolved)warnings.push(`Return control ${x.ym}: ${currentUnresolved} open-month return parts have no unambiguous posting/date identity.`);
      console.log(`Quantity control month v9 ${x.ym} open: returnEventRows=${currentEventRows.length}; unresolvedReturns=${currentUnresolved}`);
    }
  }
  ...
  if(!currentRealizationPostingAvailable){
    ...
    // business questions. v9 P&L is intentionally timed by the day the posting became
  ...
  // Finance realization is retained only as a quantity/return control on its own
  // accounting time basis. It MUST NOT define which sales belong to a delivered month.
  const sourceMonthlyValidation=validateDailyAgainstMonthly(sourceRows,controls.segments);

  // v9 P&L source of truth for SALES: the actual FBS posting that became `delivered`.
  // This fixes the month-boundary hole in v8 where a 31-Aug delivery could first appear
  // in September's finance realization and was therefore absent from August P&L.
  const deliveredSales=buildDeliveredSalesFromPostingMap(postingMap,HISTORY_START,targetEnd,maps,financeRows,sourceRows);
  const buyoutSales=appendMissingMarketplaceBuyoutSales(deliveredSales.rows,validatedBuyoutRows,postingMap,HISTORY_START,targetEnd,maps);
  deliveredSales.rows=buyoutSales.rows;
  if(buyoutSales.added.length){
    const gross=buyoutSales.added.reduce((z,r)=>z+asNum(r.soldQty,0)*asNum(r.unitPrice,0),0);
    console.log(`Validated marketplace-buyout sale fallback: added=${buyoutSales.added.length}; gross=${gross.toFixed(2)}; postings=${buyoutSales.added.map(r=>r.postingNumber).join(',')}`);
  }
  if(buyoutSales.invalid.length)warnings.push(`Validated marketplace-buyout sale fallback: ${buyoutSales.invalid.length} row(s) have invalid quantity or seller price.`);
  ...
  logUserPostingAudit(postingMap);

  for(const ym of closedMonthKeys(BUSINESS_START,TODAY)){
    const monthRows=rows.filter(r=>monthKey(r.date)===ym);
    ...
    const ps=deliveredPostingStats(deliveredSales.rows,`${ym}-01`,monthEnd(`${ym}-01`));
    console.log(`Delivered postings v9.1 ${ym}: postings=${ps.postings}; productRows=${ps.productRows}; uniqueSKU=${ps.sku}; soldUnits=${ps.units}; postingGross=${ps.gross.toFixed(2)}; missingPriceRows=${ps.missingPriceRows}`);
    console.log(`Delivered P&L revenue v9.1 ${ym}: revenue=${deliveredRevenue.toFixed(2)}; netUnits=${deliveredUnits}; revenueSKU=${revenueSkus}; rows=${monthRows.length}; priceSources=${JSON.stringify(priceSources)}`);
  }

  if(deliveredSales.excludedRows.length){
    const byMonth=new Map();
    for(const r of deliveredSales.excludedRows){const ym=monthKey(r.deliveryDate);const a=byMonth.get(ym)||[];a.push(r);byMonth.set(ym,a)}
    for(const [ym,a] of [...byMonth.entries()].sort()){
      const units=a.reduce((z,r)=>z+asNum(r.quantity,0),0),gross=a.reduce((z,r)=>z+asNum(r.quantity,0)*asNum(r.price,0),0);
      console.log(`Delivered item verification v9.1 ${ym}: excludedRows=${a.length}; excludedUnits=${units}; excludedGross=${gross.toFixed(2)}; examples=${a.slice(0,8).map(r=>`${r.postingNumber}/${r.article||r.sku}`).join(',')}`);
    }
  } else console.log('Delivered item verification v9.1: excludedRows=0');
  ...
  for(const seg of officialDaily){
    const v=validationBetweenRealizedRows(pAndLEvents,seg.rows||[],seg.start,seg.end);
  ...
  const currentStart=monthStart(TODAY);
  const finalCurrentDailyValidation=dailyControlValidation(pAndLEvents,officialDaily,currentStart,targetEnd);
  const fallbackCurrentComplete=Boolean(currentOfficialCoverage&&currentOfficialCoverage.missingDays.length===0&&currentFallbackReturnUnresolved===0);
  ...
  const pAndLPriceMissing=deliveredSales.missingPriceRows;
  const fbsDeliveryComplete=deliveryHistoryContext?.fbsComplete===true;
  const complete=closedComplete&&currentComplete&&sourceMonthlyOk&&fbsDeliveryComplete&&pAndLPriceMissing===0&&inexactDeliveryRows===0&&unresolvedDeliveryDates===0&&retro.orphanReturns.length===0;
  ...
  const orphanReturnDiagnostics=orphanReturnDiagnosticRows(retro.orphanReturns,pAndLEvents,financeRows,returnRows,postingMap);
...
let financeRows=financeRefresh.rows.length?replaceFinanceWindow(prevFinanceRows,financeRefresh.rows,financeFrom,TODAY):prevFinanceRows;
...
const compensationSupplement=await loadCompensationLinkSupplement();
const compensationLinkResult=applyValidatedCompensationLinks(financeRows,compensationSupplement,postingMap,maps);
financeRows=compensationLinkResult.rows;
if(compensationSupplement.rows.length)console.log(`Validated compensation links: linked=${compensationLinkResult.diagnostics.linked}; already=${compensationLinkResult.diagnostics.alreadyLinked}; amount=${compensationLinkResult.diagnostics.total.toFixed(2)}; ${compensationLinkResult.diagnostics.links.map(x=>`${x.postingNumber}@${x.date}`).join(',')}`);
...
if(buyoutSupplement.rows.length)console.log(`Validated marketplace-buyout supplement: rows=${buyoutSupplement.rows.length}; August commission=${buyoutSupplement.total.toFixed(2)}; revenue fallback is used only for a posting absent from Seller API delivered sales.`);
const realization=await buildDailyBusinessRealization(previous,maps,financeRows,postingMap,returnRows,returnsComplete,warnings,{fbsComplete:fbsDeliveryHistory.complete,historyStart:HISTORY_START},buyoutSupplement.rows);
const publishQualityGate=validateRealizationPublishGate(realization);
console.log(`Publish quality gate: status=${publishQualityGate.status}; rows=${publishQualityGate.rowCount}; duplicates=${publishQualityGate.duplicateRows}; invalid=${publishQualityGate.invalidNumericRows}; returnBridge=${publishQualityGate.returnOverSaleRows}; revenueBridge=${publishQualityGate.revenueBridgeRows}; buyoutCoverage=${publishQualityGate.marketplaceBuyoutCoveredPostings}/${publishQualityGate.marketplaceBuyoutExpectedPostings}`);
if(publishQualityGate.criticalIssues.length)throw new Error(`Publication blocked by data quality gate: ${publishQualityGate.criticalIssues.join('; ')}`);
...
/* Management-period attribution: every ordinary Finance operation linked to a
   realized order follows that order's sale-recognition date. Return-related
   charges and the explicitly classified common Ozon expenses remain on their
   actual Finance date. A mixed API accrual is split at charge-line level so only
   the common component leaves order/SKU economics. */
...
console.log(`Order-period Finance attribution v12: linked=${financeAttribution.diagnostics.linkedRows}; shifted=${financeAttribution.diagnostics.shiftedRows}; returns kept on Finance date=${financeAttribution.diagnostics.returnRowsKeptOnFinanceDate}; period-common kept on Finance date=${financeAttribution.diagnostics.periodCommonRowsKeptOnFinanceDate}; positive income kept on Finance date=${financeAttribution.diagnostics.positiveIncomeRowsKeptOnFinanceDate}; without sale date=${financeAttribution.diagnostics.linkedWithoutSaleDate}; ledger delta=${financeAttribution.diagnostics.totalDelta.toFixed(6)}; moves=${JSON.stringify(financeAttribution.diagnostics.monthMoves)}`);
console.log(`SKU finance by-day direct v12.0: rows=${skuFinanceRows.length}; accounting date=order sale date when linked; named common Ozon expenses stay on Finance date; source=/v1/finance/accrual/by-day`);
...
const payload={
  ...
  diagnostics:{
    ...
    financeRefreshFrom:financeFrom,financeFreshRows:financeRefresh.rows.length,financeSource:financeRefresh.source,financeRowsAll:financeRows.length,financeRowsPublished:financeRowsPublished.length,financeGrossRevenueTotal:finTotals.grossRevenue,financeNetAfterOzonTotal:finTotals.netAfterOzon,financePublishedRange:{from:realizedRange.start,to:realizedRange.end},financeAttributionVersion:FINANCE_ATTRIBUTION_VERSION,financeAttributionUpgrade,commonOzonExpenses:commonExpenseSplit.diagnostics,orderExpenseDateAttribution:financeAttribution.diagnostics,financeDirectSkuRows:financeDirectRows.length,financeUnallocatedRows:financeUnallocatedRows.length,financeDirectGross,financeDirectNet,financeUnallocatedNet,financeReconcileDelta,
    ...
    orderMetaVersion:ORDER_META_VERSION,orderSalePostings:Object.values(postingMap).filter(p=>p.status==='delivered').length,orderNumberResolved:Object.values(postingMap).filter(p=>p.orderNumber).length,orderDateResolved:Object.values(postingMap).filter(p=>p.orderDate).length,deliveryEngineVersion:14,deliveryStatusDateVersion:DELIVERY_STATUS_DATE_VERSION,fbsDeliveryComplete:fbsDeliveryHistory.complete,fbsDeliveryMissingDays:fbsDeliveryHistory.missingDays.length,fbsDeliveryScannedDays:fbsDeliveryHistory.scannedDays.length,fbsDeliveryBackfill:fbsDeliveryHistory.fullBackfill,fbsExactDeliveryPostingsRefresh:fbsDeliveryHistory.rows.length,deliveryDateExactCoverage:realization.rows.length?100*realization.rows.filter(r=>r.deliveryDateExact===true).length/realization.rows.length:null,unresolvedDeliveryDates:realization.rows.filter(r=>!r.deliveryDate).length,inexactDeliveryRows:realization.diagnostics.inexactDeliveryRows||0,financeRecognitionRows:realization.diagnostics.financeRecognitionRows||0,financeRecognitionFallbackRows:realization.diagnostics.financeRecognitionFallbackRows||0,financeRecognitionAmbiguousRows:realization.diagnostics.financeRecognitionAmbiguousRows||0,returnsApiSupplementRows:realization.diagnostics.returnsApiSupplementRows||0,returnsApiSupplementUnits:realization.diagnostics.returnsApiSupplementUnits||0,retroReturnedUnits:realization.diagnostics.retroReturnedUnits||0,retroReturnedRevenue:realization.diagnostics.retroReturnedRevenue||0,provisionalDeliveredSales:realization.rows.filter(r=>r.provisional).reduce((z,r)=>z+asNum(r.soldQty,0),0),provisionalDeliveredRevenue:realization.rows.filter(r=>r.provisional).reduce((z,r)=>z+asNum(r.revenue,0),0),currentRealizationPostingAvailable:realization.diagnostics.currentRealizationPostingAvailable,currentPostingError:realization.diagnostics.currentPostingError||'',currentFallbackValidation:realization.diagnostics.currentFallbackValidation||null,currentOfficialCoverage:realization.diagnostics.currentOfficialCoverage||null,currentFallbackPriceMissing:realization.diagnostics.currentFallbackPriceMissing||0,pAndLPriceMissing:realization.diagnostics.pAndLPriceMissing||0,deliveredPostingRows:realization.diagnostics.deliveredPostingRows||0,excludedDeliveredProductRows:realization.diagnostics.excludedDeliveredProductRows||0,excludedDeliveredProductUnits:realization.diagnostics.excludedDeliveredProductUnits||0,excludedDeliveredProductGross:realization.diagnostics.excludedDeliveredProductGross||0,excludedDeliveredProductSample:realization.diagnostics.excludedDeliveredProductSample||[],nonFbsDeliveredWithoutExactDate:realization.diagnostics.nonFbsDeliveredWithoutExactDate||0,deliveryGrossMonthlyValidation:realization.diagnostics.monthlyValidation,unmatchedReturnUnits:realization.diagnostics.orphanReturnUnits||0,prePeriodReturnUnits:realization.diagnostics.prePeriodReturnUnits||0,
...
await fs.writeFile(path.join(process.cwd(),'data','ozon-status.json'),JSON.stringify({ok:warnings.length===0,generatedAt,mode:SYNC_MODE,sourcePolicy:SOURCE_POLICY,dashboardData:'ozon-dashboard.enc.json',compression:'gzip',counts:payload.diagnostics,note:'Public status contains no API key or detailed financial rows. v10.8 serves a slim gzip+AES dashboard payload without workflow history, while ozon-data.enc.json keeps the full encrypted sync state. Product finance uses direct SKU attribution from /v1/finance/accrual/by-day plus validated supplements for August marketplace-buyout/CIS commissions and compensation-to-posting links omitted by Seller API. Confirmed delivered sales are recognized in the reporting period by the positive Finance accrual date; Posting API delivery dates remain separate audit fields. Ordinary Ozon charges linked to an order are attributed to that order sale period. Return-related charges, named common Ozon expenses and other seller-level charges remain on their actual Finance dates. Named common expenses are split from mixed order accruals, shown separately at period level and excluded from order/SKU unit economics without changing the Finance ledger total. Original Ozon Finance dates are retained for payout reconciliation. Returns API supplements return events missing from realization reports. Compensation remains separate from sales revenue and appears in linked order/SKU unit economics without changing the Finance ledger total. Revenue uses posting products.price × quantity; a validated CIS seller price is used only when that posting is completely absent from Seller API delivered sales. A blocking publish gate rejects duplicate order-item rows, broken return/revenue bridges, missing sale prices, and missing validated CIS postings.'},null,2));
